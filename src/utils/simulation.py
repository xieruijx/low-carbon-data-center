...
class Simulation(object):
    ...
    def simulate(param, sol, data):
        ...
        for t in range(data['Num_T']):
            ...
            tq_E = traj['q_E'][t]
            I = tq_F @ (a - m @ np.ones((param['Num_B'],))) + tq_B @ (m.T @ np.ones((param['Num_F'],)) - p_B) + tq_S @ (p_SC * param['eta_SC'] - p_SD / param['eta_SD']) + tq_H @ (param['alpha_B'] * p_B - param['alpha_C'] * p_C - data['beta_C'][:, t]) + tq_E * (data['gamma_E'][:, t] @ (p_B + p_SC - p_SD + p_C) - param['C_Eo'])
            J = sum(sum(param['gamma_R'] * m)) + param['gamma_F'] @ (data['A_F'][:, t] - a) + param['gamma_QF'] @ (a - m @ np.ones((param['Num_B'],))) + param['gamma_QB'] @ (m.T @ np.ones((param['Num_F'],)) - p_B) + param['gamma_S'] @ (p_SC + p_SD) + data['gamma_P'][:, t] @ (p_B + p_SC - p_SD + p_C)
